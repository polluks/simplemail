...
static int mail_compose_write_headers(FILE *fp, struct composed_mail *new_mail)
{
	...
	if ((subject = encode_header_field_utf8("Subject",new_mail->subject)))
	{
		...
		fputs(subject,fp);
		free(subject);
		fprintf(fp,"X-Mailer: SimpleMail %d.%d (%s) E-Mail Client (c) 2000-%s by Hynek Schlawack and Sebastian Bauer\n",VERSION,REVISION,SM_OPERATINGSYSTEM,SIMPLEMAIL_DATE+6);
